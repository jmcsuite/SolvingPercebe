...
int main () {
    ...
    for (int i = 0; i < N; i++) {
        if (b[i] > 0) Dinics::vadj[S][i] = b[i];
        else Dinics::vadj[i][T] = b[i]*-1;
    ...
    map<ll,ll> prev;
    for (int i = 0; i < N; i++) {
        for (ll j = 1; j*j <= vl[i]; j++) {
            if (vl[i]%j) continue;
            if (prev.find(j) != prev.end()) Dinics::vadj[i][prev[j]] = Dinics::maxFlow;
            if(vl[i]/j == j) continue;
            if (prev.find(vl[i]/j) != prev.end()) Dinics::vadj[i][prev[vl[i]/j]] = Dinics::maxFlow;
        }
        prev[vl[i]] = i;
    }

    ll mc = Dinics::dinics(N+2, S, T);
    cout << ans  - mc << '\n';
}
