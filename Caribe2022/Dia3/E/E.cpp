...
long long inv[maxinv];
    ...
    void move(pll q){
        ll j = q.second;
        ll i = q.first;
        while(der < j) add(dfspath[++der], 1);
        while(izq > i) add(dfspath[--izq], 1);
        while(der > j) add(dfspath[der--], -1);
        while(izq < i) add(dfspath[izq++], -1);
    ...
    void solve(){
        for(int i=0; i<q; i++){
            ...
            move(p);
            pll s(dfspath[p.first], dfspath[p.second]);
            ll lca = query(s.second, s.first);
            if(lca == s.second){
                ll ans = print();
...
long long ffact[maxinv];
long long invFact[maxinv];
void buildInv(){
    ffact[1]=1;
    inv[1]=1;
    for(int i=2; i<maxinv; i++) ffact[i] = (ffact[i-1]*i)%mod;
    invFact[maxinv-1] = bin_exp(ffact[maxinv-1],mod-2);
    for(int i=maxinv-2; i>=1; i--) invFact[i] = ((i+1)*invFact[i+1])%mod;
    for(int i=2; i<maxinv; i++) inv[i] = (ffact[i-1]*invFact[i])%mod;
}
...
int main(){
    ...
    while(T--){
        ...
        vpll qqueries(Q);
        for(int i=0; i<Q; i++) cin>>qqueries[i].first>>qqueries[i].second;
        for(int i=0; i<Q; i++){
            qqueries[i].first--;
            qqueries[i].second--;
        }
        ...
        ll s = sqrt(N);
        for(int i=0; i<Q; i++){
            queries[i].second = transform(qqueries[i],i);
            queries[i].first.first = queries[i].second.first/s;
        ...
        sort(queries, queries+Q,cm);
        add(0,1);

        solve();
        for(int i=0; i<Q; i++) cout << allAns[i] << '\n';
