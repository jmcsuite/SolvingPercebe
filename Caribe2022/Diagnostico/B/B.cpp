...
namespace SuffixTree{
    ...
    void setLen(){
        for(int i=1; i<=nodeCount; i++){ 
            if(len[i] == -1) childs[i] = 1;
            if(len[i] == -1) len[i] = n-idx[i]-1;
        }
    }
    ll sz[2*maxn];
    ...
    void print(){
        for(int i=1; i<= nodeCount; i++){
            cout << "Node: " << i << " len: " << len[i] << " size: " << sz[i] << " childs: " << childs[i] << endl;
    ...
    void setSz(ll x){
        for(char i='a'; i<='z'+1; i++){
            ...
            setSz(to[x][i]);
            sz[x] += sz[to[x][i]];
            sz[x] += len[to[x][i]]*childs[to[x][i]];
            childs[x] += childs[to[x][i]];
        }
    }
    ...
    void solve(ll x, ll cant){
        ...
        for(char i='a'; i<='z'+1; i++){
            if(to[x][i] == 0) continue;
            if(len[to[x][i]]*childs[to[x][i]] >= cant){
                ll maxi = cant-1;
                maxi/=childs[to[x][i]];
                maxi++;
                for(int j=idx[to[x][i]], k=0; k<maxi; j++, k++){
                    ans += s[j];
                }
                return;
            }
            if(len[to[x][i]]*childs[to[x][i]] + sz[to[x][i]] >= cant){
                for(int j=idx[to[x][i]], k=0; k<len[to[x][i]]; k++,j++){
                    ans+=s[j];
                }
                solve(to[x][i], cant-len[to[x][i]]*childs[to[x][i]]);
                return;
            }
            cant-=len[to[x][i]]*childs[to[x][i]] + sz[to[x][i]];
        }
    }
}
